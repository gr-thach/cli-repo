"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const accounts_1 = require("../../src/controllers/accounts");
const acl_1 = require("../../src/helpers/acl");
const accounts_2 = require("../../src/helpers/core-api/accounts");
const permissions_1 = require("../../src/helpers/permissions");
const interfaces_1 = require("../../src/interfaces");
jest.mock('../../src/helpers/permissions');
jest.mock('../../src/helpers/user');
jest.mock('../../src/sync/bitbucketDataCenter/syncBitbucketDataCenterAccounts');
jest.mock('../../src/helpers/core-api/accounts');
jest.mock('../../src/helpers/acl');
jest.mock('../../src/helpers/account');
jest.mock('../../config', () => ({
    ...jest.requireActual('../../config'),
    env: {
        GITHUB_API_URL: 'https://api.github.com',
        GITLAB_URL: 'https://gitlab.com',
        BITBUCKET_DATA_CENTER_API_URL: 'https://bitbucket.example.com',
        BITBUCKET_DATA_CENTER_SITE_URL: 'https://bitbucket.example.com',
        BITBUCKET_DATA_CENTER_OAUTH_CONSUMER_KEY: 'consumer',
        // This is a random generated private key.
        BITBUCKET_DATA_CENTER_OAUTH_CONSUMER_SECRET: ''
    }
}));
describe('Accounts controller', () => {
    const send = jest.fn();
    const hasAccessTo = {
        read: [],
        write: [interfaces_1.Resource.ACCOUNTS, interfaces_1.Resource.JIRA_CONFIG, interfaces_1.Resource.SUBSCRIPTION]
    };
    let res;
    beforeEach(() => {
        jest.clearAllMocks();
        jest.restoreAllMocks();
        permissions_1.getAccountPermissionForUser.mockReturnValue(hasAccessTo);
        res = {
            status: jest.fn(() => {
                return {
                    send
                };
            })
        };
    });
    afterEach(() => {
        jest.clearAllMocks();
        jest.restoreAllMocks();
    });
    const validGitlabUser = {
        gitlabNickname: 'my-gitlab-nickname',
        provider: 'gitlab'
    };
    const validBitbucketDataCenterUser = {
        bitbucketDataCenterNickname: 'my-bitbucket-data-center-nickname',
        provider: 'bitbucket_data_center'
    };
    describe('list Gitlab accounts', () => {
        it('Should return Gitlab accounts.', async () => {
            const req = { user: validGitlabUser };
            const gitlabAccounts = [
                {
                    idAccount: 100,
                    providerInternalId: '10',
                    provider: 'GITLAB',
                    type: 'ORGANIZATION',
                    hasAccessTo
                },
                {
                    idAccount: 101,
                    providerInternalId: '20',
                    provider: 'GITLAB',
                    type: 'ORGANIZATION',
                    hasAccessTo
                },
                {
                    idAccount: 102,
                    providerInternalId: '1',
                    provider: 'GITLAB',
                    type: 'USER',
                    hasAccessTo
                }
            ];
            accounts_2.findAccountsByIds.mockReturnValueOnce(gitlabAccounts);
            const allowedAccounts = { '100': {}, '101': {}, '102': {} };
            acl_1.getAllowedAccountsByUser.mockReturnValueOnce({ allowedAccounts, isSynchronizing: false });
            await (0, accounts_1.list)(req, res);
            expect(send).toBeCalledWith({
                gitlab: gitlabAccounts
            });
            expect(accounts_2.findAccountsByIds).toBeCalledWith(Object.keys(allowedAccounts).map(Number));
        });
    });
    describe('list Bitbucket data center accounts', () => {
        it('Should return Bitbucket data center accounts.', async () => {
            const req = { user: validBitbucketDataCenterUser };
            // Data received from our API.
            const userAccount = {
                idAccount: 300,
                providerInternalId: '15',
                provider: 'BITBUCKET_DATA_CENTER',
                type: 'USER',
                hasAccessTo
            };
            const bbdcAccounts = [
                {
                    idAccount: 400,
                    providerInternalId: '25',
                    provider: 'BITBUCKET_DATA_CENTER',
                    type: 'ORGANIZATION',
                    hasAccessTo,
                    url: undefined,
                    avatar_url: undefined
                },
                {
                    idAccount: 500,
                    providerInternalId: '35',
                    provider: 'BITBUCKET_DATA_CENTER',
                    type: 'ORGANIZATION',
                    hasAccessTo,
                    url: undefined,
                    avatar_url: undefined
                },
                userAccount
            ];
            accounts_2.findAccountsByIds.mockReturnValueOnce(bbdcAccounts);
            const allowedAccounts = { '300': {}, '400': {}, '500': {} };
            acl_1.getAllowedAccountsByUser.mockReturnValueOnce({ allowedAccounts, isSynchronizing: false });
            await (0, accounts_1.list)(req, res);
            expect(send).toBeCalledWith({
                bitbucket_data_center: bbdcAccounts
            });
            expect(accounts_2.findAccountsByIds).toBeCalledWith(Object.keys(allowedAccounts).map(Number));
        });
    });
});
//# sourceMappingURL=accounts.test.js.map