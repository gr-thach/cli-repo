"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const applications_1 = require("../../src/controllers/applications");
const applications_2 = require("../../src/helpers/core-api/applications");
jest.mock('../../src/helpers/core-api/applications');
jest.mock('../../src/helpers/acl');
jest.mock('../../config', () => ({
    ...jest.requireActual('../../config'),
    env: {
        GITHUB_API_URL: 'https://api.github.com',
        GITLAB_URL: 'https://gitlab.com',
        BITBUCKET_DATA_CENTER_API_URL: 'https://bitbucket.example.com',
        BITBUCKET_DATA_CENTER_SITE_URL: 'https://bitbucket.example.com',
        BITBUCKET_DATA_CENTER_OAUTH_CONSUMER_KEY: 'consumer',
        // This is a random generated private key.
        BITBUCKET_DATA_CENTER_OAUTH_CONSUMER_SECRET: ''
    }
}));
describe('Applications controller', () => {
    const send = jest.fn();
    const status = jest.fn();
    let res;
    beforeEach(() => {
        jest.clearAllMocks();
        jest.restoreAllMocks();
        res = {
            status: jest.fn(() => {
                return {
                    status,
                    send
                };
            })
        };
    });
    afterEach(() => {
        jest.clearAllMocks();
        jest.restoreAllMocks();
    });
    describe('list applications', () => {
        it('Should return applications.', async () => {
            const req = { query: { accountId: 1, limit: 0, offset: 16 } };
            const applications = [
                {
                    idApplication: 1,
                    name: 'Web',
                    description: 'Web app'
                },
                {
                    idApplication: 2,
                    name: 'Android',
                    description: 'Android app'
                },
                {
                    idApplication: 3,
                    name: 'iOS',
                    description: 'iOS app'
                }
            ];
            applications_2.queryApplications.mockReturnValueOnce({ applications, totalCount: 3 });
            await (0, applications_1.list)(req, res);
            expect(send).toBeCalledWith({ applications, totalCount: 3 });
            expect(applications_2.queryApplications).toBeCalledWith(1, { teamId: undefined, search: undefined }, 0, 16);
        });
    });
    describe('get application filters', () => {
        it('Should return application filters', async () => {
            const req = { query: { accountId: 1 } };
            const applicationFilters = {
                department: ['Development', 'Security', 'Automation']
            };
            applications_2.queryApplicationsFilters.mockReturnValueOnce(applicationFilters);
            await (0, applications_1.filters)(req, res);
            expect(send).toBeCalledWith(applicationFilters);
            expect(applications_2.queryApplicationsFilters).toBeCalledWith(1, { search: undefined, teamId: undefined });
        });
    });
    describe('get application by id', () => {
        it('Should return application by id', async () => {
            const req = { query: { accountId: 1 }, params: { applicationId: 10 } };
            const application = {
                idApplication: 10,
                name: 'Frontend',
                description: 'Frontend app'
            };
            applications_2.queryApplicationById.mockReturnValueOnce(application);
            await (0, applications_1.getById)(req, res);
            expect(send).toBeCalledWith(application);
            expect(applications_2.queryApplicationById).toBeCalledWith(1, 10);
        });
    });
    describe('create application', () => {
        it('Should create application', async () => {
            const application = {
                name: 'Frontend',
                description: 'Frontend app'
            };
            const req = { query: { accountId: 3 }, body: { application } };
            applications_2.createApplication.mockReturnValueOnce({ idApplication: 100, ...application });
            await (0, applications_1.create)(req, res);
            expect(send).toBeCalledWith({ idApplication: 100, ...application });
            expect(applications_2.createApplication).toBeCalledWith(3, application);
        });
    });
});
//# sourceMappingURL=applications.test.js.map