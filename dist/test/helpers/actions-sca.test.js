...
describe('Actions helper', () => {
    ...
    describe('upsertActionByFinding (SCA)', () => {
        it('Should throw error if account id is not set', async () => {
            expect.assertions(1);
            try {
                await upsertActionByFinding({ ...validFinding, repository: { ...validFinding.repository, fkAccount: undefined } }, validAction, validIdAccount, validUser);
            }
            catch (e) {
        ...
        it('create SCA action (without dependency version) if no existing SCA action is found', async () => {
            ...
            expect(findSingleAction).toBeCalledWith(expect.not.objectContaining({
                dependencyVersion: { equalTo: '1.8.1' }
            }));
            expect(createAction).toBeCalledWith(expect.objectContaining({
                action: validAction,
...
//# sourceMappingURL=actions-sca.test.js.map