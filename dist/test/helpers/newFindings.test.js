...
const { WONT_FIX, FALSE_POSITIVE, MARK_AS_FIXED, MARK_AS_VULNERABILITY, VULNERABILITY } = FINDING_STATUS;
        ...
        for (let y = 1; y < transitionTable.length; y++) {
            ...
            for (let x = 1; x < transitionTable[y].length; x++) {
                ...
                if (wasTransitionAllowed !== shouldTransitionBeAllowed) {
                    throw new Error(`Finding status transition from ${currentStatus} -> ${newStatus} should ${shouldTransitionBeAllowed ? 'be allowed' : 'not be allowed'}`);
                }
            }
        }
    });
...
//# sourceMappingURL=newFindings.test.js.map