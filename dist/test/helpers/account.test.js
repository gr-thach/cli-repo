...
const account_1 = __importDefault(require("../../src/helpers/account"));
const config_1 = require("../../config");
const accounts_1 = require("../../src/helpers/core-api/accounts");
const subscriptions_1 = require("../../src/helpers/core-api/subscriptions");
const subscriptionChangelogs_1 = require("../../src/helpers/core-api/subscriptionChangelogs");
const plans_1 = require("../../src/helpers/core-api/plans");
...
describe('Accounts helper', () => {
    beforeEach(() => {
        ...
        jest.resetAllMocks();
        users_1.findUserByProviderInternalId.mockReturnValueOnce({ idUser: 123 });
        config_1.env.GUARDRAILS_CLI_TOKEN_SECRET = 'dummy-cli-token-secret';
    });
    afterEach(() => {
        jest.clearAllMocks();
        jest.restoreAllMocks();
    ...
    describe('constructor', () => {
        it('Should throw error if no provider is provided', async () => {
            ...
            try {
                ...
                new account_1.default(invalidProvider, validUser);
            }
            catch (e) {
        ...
        it('Should throw error if no user is provided', async () => {
            ...
            try {
                // eslint-disable-next-line
                new account_1.default(gitlabProvider, invalidUser);
            }
            catch (e) {
    ...
    const expectAccountNotCreatedOrUpdated = () => {
        expect(accounts_1.createAccount.mock.calls.length).toEqual(0);
        expect(accounts_1.createAccounts.mock.calls.length).toEqual(0);
        expect(accounts_1.updateAccount.mock.calls.length).toEqual(0);
        expect(subscriptions_1.createSubscriptions.mock.calls.length).toEqual(0);
        expect(subscriptionChangelogs_1.createSubscriptionChangelogs).toHaveBeenCalledTimes(0);
        expect(plans_1.findPlanByCode).not.toHaveBeenCalled();
    };
    describe('syncGitlabGroups', () => {
        it("Shouldn't create accounts if no Gitlab groups is given.", async () => {
            const groups = [];
            const accountHelper = new account_1.default(gitlabProvider, validUser);
            accounts_1.findBaseAccountsByProviderInternalIds.mockReturnValueOnce(Promise.resolve([]));
            accounts_1.findBaseAccountsByProviderInternalIds.mockReturnValueOnce(Promise.resolve([]));
        ...
        it('Creating accounts for Gitlab groups should only be allowed if provider is set to Gitlab', async () => {
            ...
            const accountHelper = new account_1.default(invalidProvider, validUser);
            accounts_1.findBaseAccountsByProviderInternalIds.mockReturnValueOnce(Promise.resolve([]));
            try {
        ...
        it("Shouldn't create account if Gitlab group doesn't have path or full_path defined.", async () => {
            ...
            try {
                ...
                expect(e.message).toEqual('Expected Gitlab group (20) to have either a path or a full_path.');
                expectAccountNotCreatedOrUpdated();
            }
        });
        it("Shouldn't create account if an account already exist for a given Gitlab group.", async () => {
            ...
            const accountHelper = new account_1.default(gitlabProvider, validUser);
            accounts_1.findBaseAccountsByProviderInternalIds.mockReturnValueOnce(Promise.resolve(accounts));
            accounts_1.findBaseAccountsByProviderInternalIds.mockReturnValueOnce(Promise.resolve(accounts));
            await accountHelper.syncGitlabGroups(groups);
            expectAccountNotCreatedOrUpdated();
        });
        it('Should create account for Gitlab root-group.', async () => {
            ...
            const accountHelper = new account_1.default(gitlabProvider, validUser);
            accounts_1.findBaseAccountsByProviderInternalIds.mockReturnValueOnce(Promise.resolve([]));
            accounts_1.findBaseAccountsByProviderInternalIds.mockReturnValueOnce(Promise.resolve([createdAccount]));
            accounts_1.createAccount.mockReturnValueOnce(Promise.resolve(createdAccount));
            plans_1.findPlanByCode.mockReturnValueOnce(Promise.resolve(freePlan));
            await accountHelper.syncGitlabGroups(groups);
            expect(accounts_1.createAccount.mock.calls[0][0]).toEqual({
            ...
            expect(subscriptions_1.createSubscriptions.mock.calls[0][0]).toEqual([
                {
                    fkAccount: 101,
                    fkPlan: 99,
                    interval: 'YEARLY',
            ...
            expect(subscriptionChangelogs_1.createSubscriptionChangelogs).toHaveBeenCalledTimes(1);
            expect(plans_1.findPlanByCode).toBeCalledWith('FREE');
            expect(accounts_1.updateAccount).not.toHaveBeenCalled();
        });
        ...
        it('Should update account if full_path on Gitlab group and login on account differs.', async () => {
            ...
            const accountHelper = new account_1.default(gitlabProvider, validUser);
            accounts_1.findBaseAccountsByProviderInternalIds.mockReturnValueOnce(Promise.resolve(accounts));
            accounts_1.findBaseAccountsByProviderInternalIds.mockReturnValueOnce(Promise.resolve([updatedAccount]));
            await accountHelper.syncGitlabGroups(groups);
            expect(accounts_1.createAccount).not.toHaveBeenCalled();
            expect(accounts_1.createAccounts).not.toHaveBeenCalled();
            expect(accounts_1.updateAccount).toBeCalledWith(50, expect.objectContaining({
                login: 'path%2Fto%2Fnew-group-name'
            }));
        });
        ...
        it("Shouldn't update account if Gitlab group doesn't have path nor full_path defined", async () => {
            ...
            try {
                ...
                expectAccountNotCreatedOrUpdated();
                expect(e.message).toEqual('Expected Gitlab group (25) to have either a path or a full_path.');
            }
        });
        it("Should throw error if accounts retrieved from db doesn't have a matching Gilab group", async () => {
            ...
            try {
                ...
                expect(err.message).toBe('Expected to find a matching Gitlab group for account (51) but non was found.');
            }
        });
        it('Update multiple accounts if Gitlab groups have changed', async () => {
            ...
            const accountHelper = new account_1.default(gitlabProvider, validUser);
            accounts_1.findBaseAccountsByProviderInternalIds.mockReturnValueOnce(Promise.resolve(accounts));
            accounts_1.findBaseAccountsByProviderInternalIds.mockReturnValueOnce(Promise.resolve(updatedAccounts));
            await accountHelper.syncGitlabGroups(groups);
            expect(accounts_1.updateAccount).toHaveBeenCalledTimes(2);
            expect(accounts_1.updateAccount).toBeCalledWith(50, expect.objectContaining({
                login: 'new-group-1-name'
            }));
            expect(accounts_1.updateAccount).toBeCalledWith(51, expect.objectContaining({
                login: 'new-group-2-name'
            }));
        });
    ...
    describe('syncGitlabUser', () => {
        it('Should create account for Gitlab user.', async () => {
            ...
            const accountHelper = new account_1.default(gitlabProvider, validUser);
            accounts_1.findBaseAccountByProviderInternalId.mockReturnValueOnce(Promise.resolve(undefined));
            accounts_1.findBaseAccountByProviderInternalId.mockReturnValueOnce(Promise.resolve(undefined));
            accounts_1.findBaseAccountByProviderInternalId.mockReturnValueOnce(Promise.resolve(createdAccount));
            accounts_1.createAccount.mockReturnValueOnce(Promise.resolve(createdAccount));
            plans_1.findPlanByCode.mockReturnValueOnce(Promise.resolve(freePlan));
        ...
        it('Should update account for Gitlab user if username has changed.', async () => {
            ...
            const accountHelper = new account_1.default(gitlabProvider, validUser);
            accounts_1.findBaseAccountByProviderInternalId.mockReturnValueOnce(Promise.resolve(account));
            accounts_1.findBaseAccountByProviderInternalId.mockReturnValueOnce(Promise.resolve(updatedAccount));
            await accountHelper.syncGitlabUser(gitlabUser);
            expect(accounts_1.createAccount).not.toHaveBeenCalled();
            expect(accounts_1.updateAccount).toHaveBeenCalledTimes(1);
            expect(accounts_1.updateAccount).toBeCalledWith(300, expect.objectContaining({
                login: 'new-username'
            }));
        });
    ...
    describe('bulkCreateFromGithubInstallations', () => {
        it('Creating accounts for Github installations should only be allowed if provider is set to Github', async () => {
            ...
            try {
                ...
                expect(e.message).toEqual('bulkCreateFromGithubInstallations only works for provider = GITHUB');
            }
        });
        it("Shouldn't create accounts if no Github installations is given.", async () => {
            const installations = [];
            const accountHelper = new account_1.default(githubProvider, validUser);
            accounts_1.createAccounts.mockReturnValueOnce(Promise.resolve([]));
            await accountHelper.bulkCreateFromGithubInstallations(installations);
            expect(accounts_1.createAccounts.mock.calls[0][0]).toEqual([]);
            expect(subscriptions_1.createSubscriptions.mock.calls[0][0]).toEqual([]);
            expect(subscriptionChangelogs_1.createSubscriptionChangelogs).toHaveBeenCalledTimes(1);
        });
        it('Should create account for Github user installation.', async () => {
            ...
            const accountHelper = new account_1.default(githubProvider, validUser);
            accounts_1.createAccounts.mockReturnValueOnce(Promise.resolve([createdAccount]));
            plans_1.findPlanByCode.mockReturnValueOnce(Promise.resolve(freePlan));
            await accountHelper.bulkCreateFromGithubInstallations(installations);
            expect(accounts_1.createAccounts.mock.calls[0][0]).toEqual([
                {
        ...
        it('Should create account for Github organization installation.', async () => {
            ...
            expect(subscriptions_1.createSubscriptions.mock.calls.length).toEqual(1);
            expect(subscriptions_1.createSubscriptions.mock.calls[0][0]).toEqual([
                {
                    fkAccount: 2010,
                    fkPlan: 899,
