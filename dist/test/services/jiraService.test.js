...
describe('JiraService', () => {
    ...
    describe('validateUrl', () => {
        ...
        it('Should throw error if domain is localhost', () => {
            ...
            expectErrorMessage(() => JiraService.validateUrl('http://localhost:8080'), expectedErrorMessage);
        });
        it('Should throw error if domain is 127.0.0.1', () => {
            const expectedErrorMessage = '127.0.0.1 is an invalid hostname';
            expectErrorMessage(() => JiraService.validateUrl('http://127.0.0.1:8080'), expectedErrorMessage);
        });
        it('Should throw error if url contains query parameters', () => {
            const expectedErrorMessage = 'Url should be in the format {protocol}://{host}{path} (where path is optional). Did you mean to write "http://domain.com"?';
            expectErrorMessage(() => JiraService.validateUrl('http://domain.com?param=test'), expectedErrorMessage);
        });
        it('Should throw error if url contains username/password', () => {
            const expectedErrorMessage = 'Url should be in the format {protocol}://{host}{path} (where path is optional). Did you mean to write "http://domain.com"?';
            expectErrorMessage(() => JiraService.validateUrl('http://user:pass@domain.com'), expectedErrorMessage);
        });
        ...
        it('Should throw error if url path ends with a slash', () => {
            const expectedErrorMessage = 'Url should be in the format {protocol}://{host}{path} (where path is optional). Did you mean to write "http://domain.com/test"?';
            expectErrorMessage(() => JiraService.validateUrl('http://user:pass@domain.com/test/'), expectedErrorMessage);
        });
    });
});
//# sourceMappingURL=jiraService.test.js.map