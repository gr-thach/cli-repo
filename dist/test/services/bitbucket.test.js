...
const bitbucket_1 = __importDefault(require("../../src/services/git/bitbucket"));
...
describe('BitbucketService', () => {
    ...
    describe('pagination', () => {
        ...
        test('should get multiple pages for api resource', async () => {
            const bitbucket = new bitbucket_1.default(accessToken, nickname);
            const dummyRepositories = createDummyObjects(25);
            const repositoryUrl = `https://bitbucket.example.com/2.0/repositories/${encodeURIComponent(account.providerInternalId)}`;
        ...
        test("should throw error if pagination response doesn't contain a pagelen", async () => {
            ...
            try {
                ...
                expect(err.message).toBe("Expected 'pagelen' in pagination response to be of type number, got undefined. Is this really a paginated resource?");
            }
        });
        test("should throw error if pagination response doesn't contain a page property", async () => {
            ...
            try {
                ...
                expect(err.message).toBe("Expected 'page' in pagination response to be of type number, got undefined. Is this really a paginated resource?");
            }
        });
        test("should throw error if pagination response doesn't contain a values property", async () => {
            ...
            try {
                ...
                expect(err.message).toBe("Expected 'values' in pagination response to be an array, got undefined. Is this really a paginated resource?");
            }
        });
        test("should throw error if 'next' isn't of type string in pagination response", async () => {
            ...
            try {
                ...
                expect(err.message).toBe("Expected 'next' in pagination response to be of type string, got number. Is this really a paginated resource?");
            }
        });
    });
});
