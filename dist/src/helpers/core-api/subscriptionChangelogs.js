...
const createSubscriptionChangelogs = async (subscriptionChangelogs) => {
  ...
  `;
    const { data } = await coreAxios.post('/graphql', subscriptionChangelogs.map(x => ({ query, variables: { input: { subscriptionChangelog: x } } })));
...
//# sourceMappingURL=subscriptionChangelogs.js.map