...
const queryScans = async ({ repositoryIds, branch, limit, offset, orderBy, filters // search, sha, hasVulnerabilities, type, sender
 }) => {
...
const queryScansFilters = async ({ repositoryIds, branch, filters // search, sha, hasVulnerabilities, type, sender
 }) => {
...
const queryScanCountPerRepo = async (repositoryId) => {
  ...
  `;
    ...
    const { data: { data: { scans: { totalCount } } } } = await coreAxios.post('/graphql', { query, variables });
    return totalCount;
};
