"use strict";
const { ACCOUNT_TYPE: { ORGANIZATION, USER }, ACCOUNT_PROVIDER: { BITBUCKET_DATA_CENTER } } = require('./core-api/enums');
const assertIsBitbucketOrgProject = project => {
    ...
    if (!project.key) {
        throw new Error(`Expected Bitbucket Data Center project (${project.id}) to have a project key.`);
    }
    if (!project.name) {
        throw new Error(`Expected Bitbucket Data Center project (${project.id}) to not have a project name.`);
    }
    if (project.type !== 'NORMAL') {
        throw new Error(`Expected Bitbucket Data Center project (${project.id}) to be of type NORMAL, got ${project.type}.`);
    }
};
const assertIsBitbucketPersonalProject = personalProject => {
    ...
    if (!personalProject.key) {
        throw new Error(`Expected Bitbucket Data Center personal project (${personalProject.id}) to have a project key.`);
    }
    if (!personalProject.name) {
        throw new Error(`Expected Bitbucket Data Center personal project (${personalProject.id}) to not have a project name.`);
    }
    if (personalProject.type !== 'PERSONAL') {
        throw new Error(`Expected Bitbucket Data Center personal project (${personalProject.id}) to be of type PERSONAL, got ${personalProject.type}.`);
    }
    if (!personalProject.owner) {
        throw new Error(`Expected Bitbucket Data Center personal project (${personalProject.id}) to have an owner defined.`);
    }
    if (!personalProject.owner.id) {
        throw new Error(`Expected Bitbucket Data Center personal project (${personalProject.id}) to have an owner id defined.`);
    }
};
...
const assertAccountMatchesPersonalProject = (account, project) => {
    ...
    if (String(project.id) !== account.providerInternalId) {
        throw new Error(`Expected account (${account.idAccount}, ${account.providerInternalId}, ${account.type}, ${account.provider}) to match Bitbucket data center project (${project.id}).`);
    }
};
...
const assertAccountsHasMatchingOrgProject = (accounts, projects) => {
    accounts.forEach(account => {
        if (!containsMatchingOrgProject(account, projects)) {
            throw new Error(`Expected to find a matching Project for account (${account.idAccount}) but non was found.`);
        }
    });
};
...
//# sourceMappingURL=assertBitbucketDataCenter.js.map