...
const bundlesSchema = Joi.alternatives().try(Joi.string().valid('auto'), Joi.array()
    .min(1)
    .items(Joi.valid(...allowedLanguages), Joi.object().pattern(Joi.string().valid(...allowedLanguages), Joi.array()
    .min(1)
...
const engineRulesSchema = Joi.object().pattern(Joi.string(), Joi.object().pattern(Joi.string(), Joi.object().keys({ enable: Joi.boolean().required(), docs: Joi.string().empty('') })));
const guardrailsRulesSchema = Joi.object().pattern(Joi.string(), Joi.object().keys({
...
const schema = Joi.object().keys({
    ...
    monorepo: Joi.array()
        .allow(false, null, '')
        .items(Joi.string(), Joi.object().pattern(/./, Joi.object().keys({
        bundles: bundlesSchema,
        report: reportSchema,
        notifications: notificationsSchema,
        ignoreYmlConfig: Joi.boolean(),
...
const validateInstallationConfig = configuration => {
    const configurationObject = typeof configuration === 'string' ? yaml.safeLoad(JSON.parse(configuration)) : configuration;
...
const transformCustomConfigRules = data => {
    // Shallow copy still has side effect
    const transformedData = { ...data };
    const rulesTransformedData = data.rules.map(rule => upperCasePropertyPipeline(rule, ['severity', 'metadata.grSeverity']));
