...
const errorRedirect = (res, err) => {
    ...
    return res.redirect(`${constants.dashboardBaseUrl}?gr-saml-auth-on-premise=error&message=${message}`);
};
const okRedirect = (res, gitProvider) => {
    return res.redirect(`${constants.dashboardBaseUrl}?gr-saml-auth-on-premise=ok&git-provider=${gitProvider}`);
};
/*
...
const samlEndpointsOnPremise = passport => {
    return (Router()
        .get('/authorize/saml/on-premise/:gitProvider', passport.authenticate('saml-on-premise', { failureRedirect: '/', failureFlash: true }), (req, res) => {
        res.end();
    })
