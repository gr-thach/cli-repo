...
const validateSamlUserCallback = samlUser => {
    ...
    if (typeof nameID !== 'string') {
        throw boom.badRequest(`Expected field nameID from the saml provider to be of type string, got: ${typeof nameID}.`);
    }
    if (nameID.length === 0) {
        throw boom.badRequest('Expected length of field nameID to be greater than 0.');
    }
    if (!email) {
        throw boom.badRequest('Email attribute was not set in saml callback from saml provider. Please contact your saml administrator.');
    }
    if (typeof email !== 'string') {
        throw boom.badRequest(`Expected email attribute from the saml provider to be of type string, got: ${typeof email}. Please contact your saml administrator.`);
    }
    if (email.length === 0) {
        throw boom.badRequest('Expected length of email attribute to be greater than 0. Please contact your saml administrator.');
    }
    if (!isValidEmail(email)) {
        throw boom.badRequest(`Email attribute from saml provider '${email}' is not a valid email. Please contact your saml administrator.`);
    }
};
