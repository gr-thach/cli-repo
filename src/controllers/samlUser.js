...
const get = async (req, res) => {
  ...
  if (!samlUser) {
    throw new Error(
      `Expected to find saml user in database but none was found with id ${idSamlUser}.`
    );
  }
  ...
  if (!samlProvider) {
    throw new Error(
      `Expected to find saml provider in database but none was found with id ${fkSamlProvider}.`
    );
  }
  ...
  if (!account) {
    throw new Error(
      `Expected to find a account connected to saml provider in database but none was found with id ${samlProvider.fkAccount}.`
    );
  }
...
const linkUser = async (req, res) => {
  ...
  if (!allowedAccounts[samlProvider.fkAccount]) {
    throw boom.badRequest(
      'You are not allowed to link a git account that is not a member of the organization that the saml provider is connected to.'
    );
  }
  ...
  if (!userInDb) {
    throw new Error(
      `Expected to find user in database but none was found with internal id '${user.providerInternalId}' and git provider '${user.provider}'.`
    );
  }
