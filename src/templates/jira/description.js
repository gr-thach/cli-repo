...
const createScaDescription = ({ finding, repository, dashboardLink, providerCodeLink }) => {
  ...
  result.push(
    `- Repository: ${link(repository.name, providerCodeLink)}`,
    `- Path: ${link(path, providerCodeLink)}`
  );
  ...
  result.push(
    `- Branch: ${link(branch, providerCodeLink)}`,
    `- Dependency Name: ${dependencyName}`,
    `- Solution: ${patchedVersions ? `Upgrade to ${patchedVersions}` : 'No patch available'}`
  );
