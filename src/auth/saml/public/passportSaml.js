...
const {
  createSamlUser,
  updateSamlUser,
  findSamlUserByExternalId
} = require('../../../helpers/core-api/samlUsers');
  ...
  try {
    ...
    if (!samlProvider) {
      throw boom.badRequest(
        `Invalid callback url. No saml provider exist with id ${idSamlProvider}.`
      );
    }
...
const signInAsUser = async (idUser, idSamlUser, req) => {
  ...
  try {
    ...
    if (!(await isValidAccessToken(accessToken, user.provider, null, accessTokenSecret))) {
      ...
      throw Error(
        "Access token isn't valid on user, this could be because the access token has become stale."
      );
    }
...
const samlEndpoints = passport => {
  return (
    Router()
      .get(
        '/authorize/saml/provider/:idSamlProvider',
