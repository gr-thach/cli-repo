...
const findJiraRepositoryConfigByRepositoryId = async idRepository => {
  ...
  if (jiraRepositoryConfigs.length > 1) {
    const ids = jiraRepositoryConfigs.map(
      jiraRepositoryConfig => jiraRepositoryConfig.idJiraRepositoryConfig
    );
    throw new Error(
      `Found multiple jira repository configs with the same repository id. Jira repository config ids were [${ids}].`
    );
  }
...
const updateJiraRepositoryConfig = async (idJiraRepositoryConfig, patch) => {
  if (!patch) {
    throw new Error(
      `Patch was null or undefined when updating jira repository config '${idJiraRepositoryConfig}'.`
    );
  }
...
const deleteJiraRepositoryConfig = idJiraRepositoryConfig =>
  updateJiraRepositoryConfig(idJiraRepositoryConfig, { deletedAt: new Date().toJSON() });

module.exports = {
  createJiraRepositoryConfig: wrapper(createJiraRepositoryConfig),
  findJiraRepositoryConfig: wrapper(findJiraRepositoryConfig),
  findJiraRepositoryConfigsByJiraIntegrationId: wrapper(
