...
const assertIsGitlabGroup = group => {
  ...
  if (group.username) {
    throw new Error(
      `Expected Gitlab group (${group.id}) to not have a username defined, is this a Gitlab user and not a group?.`
    );
  }
};
...
const assertAccountMatchesGitlabGroup = (account, group) => {
  ...
  if (String(group.id) !== account.providerInternalId) {
    throw new Error(
      `Expected account (${account.idAccount}, ${account.providerInternalId}, ${account.type}, ${account.provider}) to match Gitlab group (${group.id}).`
    );
  }
};
...
const assertHasMatchingGitlabGroup = (organizationAccounts, groups) => {
  organizationAccounts.forEach(account => {
    if (!containsMatchingGitlabGroup(account, groups)) {
      throw new Error(
        `Expected to find a matching Gitlab group for account (${account.idAccount}) but non was found.`
      );
    }
  });
};
...
const assertAccountMatchesGitlabUser = (account, gitlabUser) => {
  ...
  if (String(gitlabUser.id) !== account.providerInternalId) {
    throw new Error(
      `Expected account (${account.idAccount}, ${account.providerInternalId}, ${account.type}, ${account.provider}) to match Gitlab user (${gitlabUser.id}).`
    );
  }
};
