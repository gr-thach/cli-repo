...
const {
  findJiraRepositoryConfigsByJiraIntegrationId
} = require('./core-api/jiraRepositoryConfigs');
...
const assertIsAllowedToDelete = async idJiraIntegrationConfig => {
  const jiraRepositoryConfigs = await findJiraRepositoryConfigsByJiraIntegrationId(
    idJiraIntegrationConfig
  );

  if (jiraRepositoryConfigs.length > 0) {
    const repoNames = await Promise.all(
      jiraRepositoryConfigs.map(async jiraRepositoryConfig => {
    ...
    throw boom.badRequest(
      `Unable to delete jira integration config. This config is used by jira configs stored on repositories ${repoNames.join(
        ','
      )}.`
    );
  }
...
const isValidProjectId = (jiraProjects, jiraProjectId) =>
  jiraProjects.some(project => project.id === jiraProjectId);

const isValidIssueTypeId = (jiraIssueTypes, jiraIssueTypeId) =>
  jiraIssueTypes.some(issueType => issueType.id === jiraIssueTypeId);
