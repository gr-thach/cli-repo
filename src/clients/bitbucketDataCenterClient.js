...
class BitbucketDataCenterClient {
  ...
  createUrl(path, params) {
    ...
    if (path.includes('?')) {
      throw new Error(
        "The path shouldn't contain a '?' character, you should add query parameters to the 'params' parameter instead."
      );
    }
  ...
  validatePaginatedResponse(data) {
    if (!Array.isArray(data.values)) {
      throw new Error(
        `Expected paginated values field to be of type array, got ${typeof data.values}.`
      );
    }

    if (typeof data.isLastPage !== 'boolean') {
      throw new Error(
        `Expected 'isLastPage' to be a boolean, got ${typeof data.isLastPage}. Is this really a paginated resource?`
      );
    }
    ...
    if (!data.isLastPage) {
      if (typeof data.nextPageStart !== 'number') {
        throw new Error(
          `Expected 'nextPageStart' to be a number, got ${typeof data.nextPageStart}. Is this really a paginated resource?`
        );
      }
    }
  }
