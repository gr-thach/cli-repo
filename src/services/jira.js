...
class JiraService {
  ...
  static validateUrl(urlStr) {
    ...
    if (urlStr !== `${url.protocol}//${url.host}${pathName}`) {
      ...
      throw new Error(
        `Url should be in the format {protocol}://{host}{path} (where path is optional). Did you mean to write "${url.protocol}//${url.host}${pathName}"?`
      );
    }
  }
  ...
  async getProject(jiraProjectId) {
    if (!jiraProjectId) {
      throw new Error(
        `Unable to get Jira project. Jira project id is not defined (${jiraProjectId}).`
      );
    }
  ...
  handleError(err) {
    if (err.statusCode === 401) {
      throw new JiraServerError(
        'Unable to authenticate with Jira. Api url, username and/or password is invalid.',
        err.statusCode,
        err
      );
    } else if (err.statusCode === 404) {
      throw new JiraServerError(
        'Received status code 404 (Not Found) from Jira. Is the api url not correctly configured or does the resource not exist?',
        err.statusCode,
        err
      );
    } else if (err.cause && err.cause.code === 'ENOTFOUND') {
      throw new JiraServerError(
        'Unable to connect to Jira server. Is the api url correct?',
        err.statusCode,
        err
      );
    } else {
      throw new JiraServerError(
        `Received an unexpected error from Jira server, got response with status code ${err.statusCode}.`,
        err.statusCode,
        err
      );
    }
