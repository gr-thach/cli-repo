...
describe('PreHook controller', () => {
  ...
  describe('trigger scan', () => {
    it('Should trigger scan using cli token', async () => {
      ...
      triggerScan.mockReturnValueOnce(idScan);

      await trigger(req, res);

      expect(triggerScan).toBeCalledWith(
        sha,
        branch,
        repository.account,
        repository,
      ...
      expect(send).toBeCalledWith({
        idScan,
        dashboardUrl:
          'https://dashboard.dev.guardrails.io/bbdc/PROJECT_1/repos/1/scans?sha=864703b04dc567f06d46228eda9e24896229bef5'
      });
    });
